...
class Paraphrase_Maker:
    ...
    def lemmatize_a_word(self,p):
        ...
        while pos[i_change][1] not in ["VBZ","VBG","NNS","VBD"] or toks[i_change] in ["being","is","are","were","was","am","savings"]:
            i_change = random.choice(range(0,len(pos)))
            i_looks += 1
            if i_looks > 10:
                found_lem = False
                break
        if found_lem:
            toks[i_change] = wordnet_lemmatizer.lemmatize(toks[i_change],pos=pos[i_change][1].lower()[0])
...
def parse_arguments():
    ...
    parser.add_argument('--regex_tsv', help='path to regex file', required=True)
    parser.add_argument('--general_rules_tsv', help='path to general rules', required=True)
    parser.add_argument('--seeds_tsv', help='path to seeds', required=True)
    parser.add_argument('--end_number', help='list of possible string replacements', required=True)
    parser.add_argument('--patterns_json', help='json of spacy patterns', required=True)
...
paraphrase_maker = Paraphrase_Maker(args.regex_tsv, args.general_rules_tsv, args.seeds_tsv, args.patterns_json)
select_labels = list(paraphrase_maker.non_seeds_counter.keys())
transformers = [paraphrase_maker.get_pattern_chunks,paraphrase_maker.lemmatize_a_word,paraphrase_maker.replace_from_general_rules, paraphrase_maker.generate_from_regex]

woutfile = open("out","w")
while min(paraphrase_maker.non_seeds_counter.values()) < int(args.end_number):
    select_labels = [label for label in select_labels if paraphrase_maker.non_seeds_counter[label] < int(args.end_number)]
    ...
    new_paraphrase = paraphrase_maker.create_paraphrase(paraphrase_in,transformer,{})
    
    if new_paraphrase.text not in paraphrase_maker.existing_texts:
        #print(paraphrase_in.label +": " + str(transformer.__name__) + ": " +   " --> ".join([paraphrase_in.text, new_paraphrase_t]))


        #new_paraphrase = Paraphrase({}, transformer, paraphrase_in.label, new_paraphrase_t)
        paraphrase_maker.paraphrase_adder(new_paraphrase)
        print(new_paraphrase.get_string())
        woutfile.write("\t".join([new_paraphrase.label,new_paraphrase.text]) + "\n")
